...
`groups` 决定导出**哪些表和哪些字段**，`topModule` 决定生成代码**叫什么**。
这两件事正交，一个 target 可以只用其中一个。

字段级过滤容易被忽略：同一张表在 `client` 和 `server` 下可以有不同的列，
嵌套 bean 里的字段也一样会被过滤。示例语料的 `test.DemoGroup` 就是这个用例
（`x5` 是个嵌套 bean，它自己的字段也按组筛），回归会逐字段验证。
