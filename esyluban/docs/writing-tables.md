...
**不该在这里找什么**：嵌套结构、多态、校验器、schema 定义文件 —— 在
...
`##type` 写 `list,<元素类型>`，`##var` 里的字段名后面挂上分隔符，一个格子里填多个值：
...
`#sep=` 后面的分隔符跟这一列的数据对得上就行，`;`、`,`、`|` 都常见。
`#` 和 `&` 要转义才能当分隔符，别用它们。

### map（键值对）

`##type` 写 `map,<键类型>,<值类型>`，同样挂 `#sep=`。一个格子里
**键、值、键、值交替排列**：

|  | A | B | C |
|---|---|---|---|
| **2** | `##var` | `id` | `props#sep=,` |
| **3** | `##type` | `int` | `map,int,string` |
| **4** | | 1 | `1,apple,2,banana` |

这一格的意思是 `{1: "apple", 2: "banana"}`。键和值必须成对，落单会报错。
