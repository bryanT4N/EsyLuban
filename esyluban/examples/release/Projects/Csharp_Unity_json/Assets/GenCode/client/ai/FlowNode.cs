...
namespace dataTables.ai
{
public abstract partial class FlowNode : ai.Node
{
    public FlowNode(JSONNode _buf)  : base(_buf) 
    {
        { var __json0 = _buf["decorators"]; if(!__json0.IsArray) { throw new SerializationException(); } Decorators = new System.Collections.Generic.List<ai.Decorator>(__json0.Count); foreach(JSONNode __e0 in __json0.Children) { ai.Decorator __v0;  { if(!__e0.IsObject) { throw new SerializationException(); }  __v0 = global::dataTables.ai.Decorator.DeserializeDecorator(__e0);  }  Decorators.Add(__v0); }   }
        { var __json0 = _buf["services"]; if(!__json0.IsArray) { throw new SerializationException(); } Services = new System.Collections.Generic.List<ai.Service>(__json0.Count); foreach(JSONNode __e0 in __json0.Children) { ai.Service __v0;  { if(!__e0.IsObject) { throw new SerializationException(); }  __v0 = global::dataTables.ai.Service.DeserializeService(__e0);  }  Services.Add(__v0); }   }
    }
