...
public sealed partial class TestSep : Luban.BeanBase
{
    public TestSep(JSONNode _buf) 
    {
        ...
        { if(!_buf["x2"].IsObject) { throw new SerializationException(); }  X2 = global::dataTables.test.SepBean1.DeserializeSepBean1(_buf["x2"]);  }
        { if(!_buf["x3"].IsObject) { throw new SerializationException(); }  X3 = global::dataTables.test.SepVector.DeserializeSepVector(_buf["x3"]);  }
        { var __json0 = _buf["x4"]; if(!__json0.IsArray) { throw new SerializationException(); } X4 = new System.Collections.Generic.List<test.SepVector>(__json0.Count); foreach(JSONNode __e0 in __json0.Children) { test.SepVector __v0;  { if(!__e0.IsObject) { throw new SerializationException(); }  __v0 = global::dataTables.test.SepVector.DeserializeSepVector(__e0);  }  X4.Add(__v0); }   }
        { var __json0 = _buf["x6"]; if(!__json0.IsArray) { throw new SerializationException(); } X6 = new System.Collections.Generic.List<test.SepBean1>(__json0.Count); foreach(JSONNode __e0 in __json0.Children) { test.SepBean1 __v0;  { if(!__e0.IsObject) { throw new SerializationException(); }  __v0 = global::dataTables.test.SepBean1.DeserializeSepBean1(__e0);  }  X6.Add(__v0); }   }
    }
