...
namespace cfg.test
{
public sealed partial class AutoImport3 : Luban.BeanBase
{
    public AutoImport3(JSONNode _buf) 
    {
        ...
        { if(!_buf["x14"].IsObject) { throw new SerializationException(); }  X14 = global::cfg.test.DemoDynamic.DeserializeDemoDynamic(_buf["x14"]);  }
        { if(!_buf["x15"].IsObject) { throw new SerializationException(); }  X15 = global::cfg.test.Shape.DeserializeShape(_buf["x15"]);  }
        { if(!_buf["v2"].IsObject) { throw new SerializationException(); }  V2 = global::cfg.vec2.Deserializevec2(_buf["v2"]);  }
        ...
        { var __json0 = _buf["k10"]; if(!__json0.IsArray) { throw new SerializationException(); } K10 = new System.Collections.Generic.List<vec3>(__json0.Count); foreach(JSONNode __e0 in __json0.Children) { vec3 __v0;  { if(!__e0.IsObject) { throw new SerializationException(); }  __v0 = global::cfg.vec3.Deserializevec3(__e0);  }  K10.Add(__v0); }   }
        { var __json0 = _buf["k11"]; if(!__json0.IsArray) { throw new SerializationException(); } K11 = new System.Collections.Generic.List<vec4>(__json0.Count); foreach(JSONNode __e0 in __json0.Children) { vec4 __v0;  { if(!__e0.IsObject) { throw new SerializationException(); }  __v0 = global::cfg.vec4.Deserializevec4(__e0);  }  K11.Add(__v0); }   }
        { var _j = _buf["v11"]; if (_j.Tag != JSONNodeType.None && _j.Tag != JSONNodeType.NullValue) { { if(!_j.IsObject) { throw new SerializationException(); }  V11 = global::cfg.vec3.Deserializevec3(_j);  } } else { V11 = null; } }
    }

    public static AutoImport3 DeserializeAutoImport3(JSONNode _buf)
    {
        return new test.AutoImport3(_buf);
    }
    ...
    public const int __ID__ = 417472771;
