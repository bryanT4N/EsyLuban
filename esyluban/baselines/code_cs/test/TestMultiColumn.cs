...
public sealed partial class TestMultiColumn : Luban.BeanBase
{
    public TestMultiColumn(JSONNode _buf) 
    {
        ...
        { if(!_buf["a"].IsObject) { throw new SerializationException(); }  A = global::cfg.test.Foo.DeserializeFoo(_buf["a"]);  }
        { if(!_buf["b"].IsObject) { throw new SerializationException(); }  B = global::cfg.test.Foo.DeserializeFoo(_buf["b"]);  }
        { if(!_buf["c"].IsObject) { throw new SerializationException(); }  C = global::cfg.test.Foo.DeserializeFoo(_buf["c"]);  }
    }
